...
public class StreamingEngine extends AbstractSparkLayer {
    ...
    public void start() {
        ...
        streamingContext = new JavaStreamingContext(sparkConf,
                seconds(Long.parseLong(config.getStreamingBatchIntervalInSec())));
        JavaInputDStream<MessageAndMetadata<String, byte[]>> dStream = buildInputDStream(streamingContext);
        JavaPairDStream<String, byte[]> pairDStream = dStream.window(Durations.seconds(60), Durations.seconds(60)).mapToPair(km -> new Tuple2<>(km.key(), km.message()));
